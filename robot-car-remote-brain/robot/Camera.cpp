// MJPEG-over-HTTP camera stream, same as robot-car-shared-wifi's - the
// only HTTP surface on the robot. The consumer here is not a browser but
// brain/brain.py, which pulls frames from /stream and runs person
// detection on them.
...
void initCameraStream() {
  ...
  s->set_vflip(s, 1);
  s->set_hmirror(s, 1); // raw stream stays mirrored; the brain flips it back (cv2.flip)

  httpd_config_t httpd_cfg = HTTPD_DEFAULT_CONFIG();
  httpd_cfg.server_port = 81;
  httpd_cfg.ctrl_port = 32081;
  httpd_cfg.max_open_sockets = 2;    // single viewer at a time
  httpd_cfg.lru_purge_enable = true; // kick the stalest client instead of refusing new ones
